...
void exhaustTimers(void) {
   SysTimer timer;
   static int count = 3;               // two timers in use when this is called in loop(), plus the one in this function
...
void loop(void) {
   ...
   mytimer.attachInterrupt((CallbackArg)isr, (void*)1);
   mytimer.arm(false);
